...
import Orbits.System (System, name, bodies, names, time, velocity, position, v3InUnit)
import Orbits.Units (simTime, simEnergy, simVelocity, simLength, day, year, inUnit)
import Orbits.Simulation (setBodies, getBodies, getEnergy, doStep)
  ...
  withFile fileName WriteMode $ \file ->
    forM_ history $ \sys ->
      let t = sys^.time.inUnit simTime
          ...
          position' = v3InUnit simLength $ body^.position
...
main = TF.runSession $ do
    ...
    sim <- TF.build $ createSimulation $ V.length $ system0^.bodies

    (sim^.setBodies) (system0^.bodies)
    ...
    systems <- forM ([0..numOutputs] :: [Int]) $ \i -> do
      ...
      return $ system0 & time .~ (fromIntegral (i*stepsPerOutput) *~ D.one) D.* dt
        & bodies .~ newBodies
